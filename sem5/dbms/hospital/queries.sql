select pt.name, pt.PCP, ph.name from Patient pt inner join PhysicianE ph on pt.PCP=ph.EmployeeID where pt.pcp not in (select p.Head from Department p);

select pt.Name as "Patient",ph.name as "Physician" from Patient pt inner join PhysicianE ph on pt.PCP=ph.EmployeeID where pt.SSN in (select a.Patient from Appointment a where a.PrepNurse in (select n.EmployeeID from Nurse n where n.Registered=1) group by a.Patient having count(a.Patient) >= 2);
...
select pt.Name as "Patient", ph.name as "Physician" from Patient pt inner join PhysicianE ph on pt.PCP=ph.EmployeeID where pt.SSN in (select Patient from Undergoes where Procedures in (select pr.Code from Procedures pr where pr.Cost > 5000));
