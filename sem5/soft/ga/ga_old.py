...
def train(train_set, train_set_classes, test_set, test_set_classes):
    ...
    for i in range(len(train_set)):
        if train_set_classes[i] == yes_string or train_set_classes[i] == 'Iris-versicolor':
            n_yes += 1
        elif train_set_classes[i] == no_string or train_set_classes[i] == 'Iris-setosa':
            n_no += 1
    ...
    for j in range(len(train_set)):
        for i in range(0, len_features):
            if train_set[j][i] == '1' and (train_set_classes[i] == yes_string or train_set_classes[i] == 'Iris-versicolor'):
                f1_yes[i] += 1
            elif train_set[j][i] == '1' and ( train_set_classes[i] == no_string or train_set_classes[i] == 'Iris-setosa'):
                f1_no[i] += 1
            elif train_set[j][i] == '0' and (train_set_classes[i] == yes_string or train_set_classes[i] == 'Iris-versicolor'):
                f0_yes[i] += 1
            elif train_set[j][i] == '0' and  (train_set_classes[i] == no_string or train_set_classes[i] == 'Iris-setosa'):
                f0_no[i] += 1
...
def genetic_algorithm(pop_size, num_features, dataset, len_features):
    ...
    print('final',absolute_best_chromosome, avg )
...
def main():
    ...
    print("Baseline acc (without GA): ",naive_bayes_classifier(dataset,len_features,test_chromosome))
    print("intializing population for GA")
